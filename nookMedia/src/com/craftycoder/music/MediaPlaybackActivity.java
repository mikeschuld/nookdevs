...
public class MediaPlaybackActivity extends nookBaseActivity implements MusicUtils.Defs, GreyBarTouchInterface
{
    ...
    public void onCreate(Bundle icicle)
    {
        ...
        View b;
        b = (View) findViewById(R.id.audio_player_touchsurface);	
   ...
   /*     volume_label = (TextView) findViewById(R.id.volume);
    ...
    public boolean button0(View v, MotionEvent event) {
    	if(event.getAction() == MotionEvent.ACTION_DOWN) {
    		findViewById(R.id.party_shuffle_holder).setPressed(true);
    		return true;
    	} else if(event.getAction() == MotionEvent.ACTION_UP) {
			findViewById(R.id.party_shuffle_holder).setPressed(false);
			int shuffle;
			try {
				shuffle = MusicUtils.sService.getShuffleMode();
	            if (shuffle == MediaPlaybackService.SHUFFLE_AUTO) {
	                MusicUtils.sService.setShuffleMode(MediaPlaybackService.SHUFFLE_NONE);
	                ((android.widget.TextView) findViewById(R.id.party_shuffle_button)).setText(R.string.party_shuffle);
	            } else {
	                MusicUtils.sService.setShuffleMode(MediaPlaybackService.SHUFFLE_AUTO);
	                ((android.widget.TextView) findViewById(R.id.party_shuffle_button)).setText(R.string.party_shuffle_off);
	            }
	...
	public boolean button1(View v, MotionEvent event) {
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			findViewById(R.id.library_holder).setPressed(true);
    		return true;
		} else if(event.getAction() == MotionEvent.ACTION_UP) {	
            findViewById(R.id.library_holder).setPressed(false);
    		Intent intent;
    		intent = new Intent();
	...
	public boolean button2(View v, MotionEvent event) {
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			findViewById(R.id.delete_holder).setPressed(true);
    		return true;
		} else if(event.getAction() == MotionEvent.ACTION_UP) {
    		findViewById(R.id.delete_holder).setPressed(false);
			try {
				Intent intent;
				if (mService != null) {
				    int [] list = new int[1];
				    ...
				    Bundle b = new Bundle();
				    b.putString("description", getString(R.string.delete_song_desc,mService.getTrackName()));
	...
	public boolean button3(View v, MotionEvent event) {
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			findViewById(R.id.add_to_playlist_holder).setPressed(true);
    		return true;
    	} else if(event.getAction() == MotionEvent.ACTION_UP) {
    		findViewById(R.id.add_to_playlist_holder).setPressed(false);	
    		setupMenu();
    ...
    private void setupMenu() {
        ...
        if (MusicUtils.getCurrentAudioId() >= 0) {
        	optionMenu.clear();
            SubMenu sub = optionMenu.addSubMenu(0, ADD_TO_PLAYLIST, 0,R.string.add_to_playlist);
            MusicUtils.makePlaylistMenu(this, sub);
        }    	
    }
    @Override
    public boolean onPrepareOptionsMenu(Menu menu) {
        menu.performIdentifierAction(ADD_TO_PLAYLIST, 0);
        menu.clear();
        return true;
    }
    private void setSuffleText() {
		int shuffle;
		try {
			shuffle = MusicUtils.sService.getShuffleMode();
		} catch (Throwable e) {
...
*/
	        ...
	        case NOOK_PAGE_UP_KEY_RIGHT:
	        	...
	        	super.onKeyDown(keyCode,new KeyEvent(event));
	        	break;
