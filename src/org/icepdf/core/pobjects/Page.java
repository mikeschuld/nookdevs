...
/**
 * <p>
 * This class represents the leaves of a <code>PageTree</code> object known as
 * <code>Page</code> class. The page dictionary specifies attributes of the
 * single page of the document. Many of the page's attributes are inherited from
 * the page tree dictionary if not specified in the page dictionary.
 * </p>
 * <p/>
 * <p>
 * The page object also provides a method which will extract a page's content,
 * such as text and images. The <code>paint</code> method is the core of the
 * ICEpdf renderer, allowing page content to be painted to any Java graphics
 * context.
 * </p>
 * <p/>
 * <p>
 * Page objects in a PDF document have different boundaries defined which govern
 * various aspects of the pre-press process, such as cropping, bleed, and
 * trimming. Facilities for including printer's marks, such a registration
 ...
 * box which is what most viewer applications should use. However, if your
 * application requires the use of different page boundaries, they can be
 * specified when using the getSize or paint methods. If in doubt, always use
 * the crop box constant.
 * </p>
 * 
...
public class Page extends Dictionary implements MemoryManageable {
    ...
    /**
     * Defines the visible region of the default user space. When the page is
     * displayed or printed, its contents are to be clipped to this rectangle
     * and then imposed on the output medium in some implementation defined
     * manner.
     */
    ...
    /**
     * Create a new Page object. A page object represents a PDF object that has
     * the name page associated with it. It also conceptually represents a page
     * entity and all of it child elements that are associated with it.
     * 
     * @param l
     *            pointer to default library containing all document objects
     * @param h
     *            hashtable containing all of the dictionary entries
     */
     ...
     * 
     * @param cache
     *            if true, cached files are removed; otherwise, objects are
     *            freed but object caches are left intact.
     */
    protected synchronized void dispose(boolean cache) {
        ...
        if (isInited) {
            ...
            // work through contents and null any stream that have images in
            // them
            if (contents != null) {
    ...
    private void initPageContents() throws InterruptedException {
        ...
        if (pageContent instanceof Stream) {
            ...
            tmpStream.setPObjectReference(library.getObjectReference(entries, "Contents"));
        ...
        // if a vector, process it as needed
            ...
            for (int i = 0; i < sz; i++) {
                if (Thread.interrupted()) { throw new InterruptedException(
                    "Page Content initialization thread interrupted"); }
    ...
    private void initPageResources() throws InterruptedException {
        ...
        if (res == null) {
            ...
            while (pt != null) {
                if (Thread.interrupted()) { throw new InterruptedException(
                    "Page Resource initialization thread interrupted"); }
    ...
    private void initPageAnnotations() throws InterruptedException {
        ...
        if (annots != null && annots instanceof Vector) {
            ...
            for (int i = 0; i < v.size(); i++) {
                ...
                else if (annotObj instanceof Hashtable) { // Hashtable lacks
                    // "Type"->"Annot"
                    // entry
    ...
    public synchronized void init() {
        try {
            ...
            if (isInited) { return; }
            // try { throw new RuntimeException("Page.init() ****"); }
             ...
             * the resourse streams together so that the content parser can go
             * to town and build all of the page's shapes.
             */
            ...
            if (contents != null) {
                ...
                for (Stream stream : contents) {
                     ...
                     * String content = Utils.getContentAndReplaceInputStream(
                     * inArray, false ); input = inArray[0];
            ...
            // keeps shapes vector so we can paint what we have but make init
            // state as false
     ...
     * assumed that the annotation has a valid object reference. This is
     * commonly used with the undo/redo state manager in the RI. Use the method
     * 
     * @link{#createAnnotation for creating new annotations.
     * 
     * @param newAnnotation
    ...
    public Annotation addAnnotation(Annotation newAnnotation) {
        ...
        if (!isAnnotAReference && annots != null) {
            ...
            if (annots instanceof Vector) {
                ...
                stateManager.addChange(new PObject(annots, library.getObjectReference(entries, ANNOTS_KEY.getName())));
            }
        }
     ...
     * annotation was origional then either the page or the annot ref object is
     * also added to the state maanger. If the annotation was new then we just
     * have to update the page and or annot reference as the objects will
     * allready be in the state manager.
     */
    public void deleteAnnotation(Annotation annot) {
        ...
        // mark the item as deleted so the state manager can clean up the
        // reference.
        annot.setDeleted(true);
        
        // check to see if this is an existing annotations, if the annotations
        // is existing then we have to mark either the page or annot ref as
        // chagned.
    ...
    /**
     * Updates the annotation associated with this page. If the annotation is
     * not in this page then the annotation is no added.
     * 
     * @param annotation
     *            annotation object that should be updated for this page.
    ...
    public boolean updateAnnotation(Annotation annotation) {
        ...
        Vector<Reference> annots = (Vector) library.getObject(entries, ANNOTS_KEY.getName());
    ...
    /**
     * Gets a reference to the page's parent page tree. A reference can be
     * resolved by the Library class.
     * 
     ...
     * factor. This method will normalize rotation factors to be in the range of
     * 0 to 360 degrees.
     * 
     * @param userRotation
     *            rotation factor to be applied to page
     * @return Total Rotation, representing pageRoation + user rotation factor
    ...
    /**
     * Gets all annotation information associated with this page. Each entry in
     * the vector represents one annotation. The size of the vector represents
     * the total number of annotations associated with the page.
     * 
     * @return annotation associated with page; null, if there are no
     *         annotations.
     */
    ...
    /**
     * Gest the PageText data structure for this page. PageText is made up of
     * lines, words and glyphs which can be used for searches, text extraction
     * and text highlighting. The coordinates system has been normalized to page
     * space.
     * 
     ...
     * parsing technique that ignores some text elements. This method should be
     * used for straight text extraction.
     * 
    ...
    public synchronized PageText getText() {
        ...
        if (isInited) {
            if (shapes != null && shapes.getPageText() != null) { return shapes.getPageText(); }
        }
        ...
        try {
            /**
             * Finally iterate through the contents vector and concat all of the
             * the resouse streams together so that the contant parser can go to
             * town and build all of the pages shapes.
             */
            if (contents == null) {
                // Get the value of the page's content entry
                initPageContents();
    ...
    /**
     * Gets a vector of Images where each index represents an image inside this
     * page.
     * 
